...
class AdversarialExmaples:
    ...
    def train_P(self):
        language_model = ngrams(self.X_train, cs.NGRAM_ORDER) # TODO order of language model
    ...
    def J(self, x, y):
        ...
        return self.target_classifier.predict(x)[abs(y - 1)]
    ...
    def algoritm1_greedy_opt_strategy_for_finding_adversarial_examples(self, x, y):
        ...
        # TODO prejst len raz cele X?

        while self.J(x, y) < cs.TAU and (changed/num_words) < cs.DELTA:

            working_set = []
            for i in range(len(x)):
                x_stripe = [a for a in x]
                for w_stripe in self.embedding_model.wv.most_similar([x[i]], topn=cs.NEIGHBORHOOD):
                    if w_stripe not in find_closest_embeddings(x[i]): continue
                    x_stripe[i] = w_stripe
